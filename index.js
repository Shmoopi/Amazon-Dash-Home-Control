...
var dashFunction = function(dash_id) {
    ...
    // Got the button console.log('Found Button: ' + button.Name); Check if there
    // are any WeMo Devices to Toggle
    ...
    if (button.Hue !== undefined && button.Hue instanceof Array && button.Hue.length > 0) {
        ...
        // Load up the bridge from the device settings - passing the hueDevices variable
...
function loopWeMo(wemoDevices) {
    ...
    for (var i = 0; i < x; i++) {
        ...
        if (wemoIP !== undefined && wemoPort !== undefined) {
            ...
            // We already have the ip and port, let's create the switch and flip the binary
            // state
